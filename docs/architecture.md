...
### Local GCP emulation (no GKE/GCP account needed)

The fake transports (above) are for fast, deterministic tests — they never touch a real signing or network path. To exercise the *real* `XmlMultipartTransport`/`PubSubOutboxPublisher` code end to end without a GKE cluster or a GCP account, `docker-compose.yml` runs [fake-gcs-server](https://github.com/fsouza/fake-gcs-server) and the official Pub/Sub emulator alongside Postgres. Two config knobs, both empty by default (production behavior untouched), switch each real adapter from talking to Google's APIs to talking to the local emulator:

- **`fileintake.upload.gcs-host`** (`GCS_HOST` env var) — when set, `StorageConfig.storage()` builds the `Storage` client against that host with `NoCredentials` instead of Application Default Credentials (the emulator doesn't check auth), and auto-creates the configured bucket if it's missing. `Storage.signUrl()` still needs *something* that can produce a signature, so a second bean, `localUrlSigner`, exposes a throwaway in-memory-generated RSA `ServiceAccountSigner` — present only in emulator mode (gated by `@ConditionalOnExpression`, since the property's empty-string default means a plain `@ConditionalOnProperty` presence check would always match). `XmlMultipartTransport` takes this signer as `Optional<ServiceAccountSigner>` and passes it to `signUrl()` via `SignUrlOption.signWith(...)` only when present; it also rewrites the signed URL's scheme from `https://` to `http://` in that case only, since `signUrl()` always emits `https://` regardless of the client's configured host and fake-gcs-server serves plain HTTP. fake-gcs-server doesn't validate the signature, only that the URL is shaped like one — the keypair is generated fresh on every startup and grants access to nothing real.
- **`fileintake.outbox.pubsub-emulator-host`** (`PUBSUB_EMULATOR_HOST` env var) — when set, `PubSubConfig.pubSubPublisher()` points a plaintext gRPC channel at the emulator with `NoCredentialsProvider` instead of resolving Application Default Credentials, and creates the configured topic via `TopicAdminClient` on the same channel if it doesn't already exist (the emulator starts empty).

Both branches log a `WARN` on startup ("Using GCS/Pub-Sub emulator at ... — not for production use") so emulator mode is never silently active. Verified manually end to end: real signed-URL PUT against fake-gcs-server, `/complete` reading back real crc32c/size/generation, `/commit` inserting an outbox row, and the `worker`-profile `OutboxPoller` publishing it to the Pub/Sub emulator — pulled back via a raw subscription to confirm the payload round-trips correctly.
